...
modelingset=["../../crosslinks/Exp1-BP-CXMS-12aout16-KFG.csv","../../crosslinks/Exp2-KFG-081016-2.csv",
...
validationset=["../../crosslinks/KFGE-purif(05-25ug-180min).csv",
               "../../crosslinks/KFGE-purif(10-50ug-180min).csv",
               "../../crosslinks/KFGE-purif(10-25ug-180min).csv"]
               
uselist=validationset
if uselist==validationset:
    label="validation"
if uselist==modelingset:
    label="modeling"


for csv in validationset:
...
xlt1.load_rmf_coordinates("output_test/rmfs/0.rmf3",2650,list(IMP.pmi.tools.OrderedSet(protx+proty)),nomap=True)

#dictionary_of_gaps={}

xlt1.load_crosslinks(fcldb)
xlt1.compute_distances()
fcldb.save_csv("xl_distances_"+suffix+"."+label+".csv")
fcldb.plot("output.pdf",type="scatter",xkey="MinAmbiguousDistance",ykey=cldbkc.redundancy_key,colorkey=cldbkc.id_score_key)
xlt1.save_rmf_snapshot("bestsnapshot_"+suffix+"."+label+".rmf",color_id=cldbkc.redundancy_key)
